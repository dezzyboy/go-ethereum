...
- `-eth-network <mainnet/rinkeby/stormborn/ropsten>` filters nodes by "eth" ENR entry
...
1. initialize the aegon node with the `genesis.json` file contained in the `testdata` directory
2. import the `halfchain.rlp` file in the `testdata` directory
3. run aegon with the following flags:
```
aegon --datadir <datadir> --nodiscover --nat=none --networkid 19763 --verbosity 5
```

Then, run the following command, replacing `<enode>` with the enode of the aegon node:
 ```
...
To run the eth66 protocol test suite, initialize a aegon node as described above and run the following command,
replacing `<enode>` with the enode of the aegon node:
...
[dns-tutorial]: https://aegon.ethereum.org/docs/developers/dns-discovery-setup
