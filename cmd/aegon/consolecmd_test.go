...
func TestConsoleWelcome(t *testing.T) {
	...
	// Start a aegon console, make sure it's cleaned up and terminate the console
	aegon := runMinimalGeth(t, "--miner.etherbase", coinbase, "console")
	...
	aegon.SetTemplateFunc("apis", func() string { return ipcAPIs })
...
func TestAttachWelcome(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		ipc = `\\.\pipe\aegon` + strconv.Itoa(trulyRandInt(100000, 999999))
	} else {
		ipc = filepath.Join(t.TempDir(), "aegon.ipc")
	}
	// And HTTP + WS attachment
	p := trulyRandInt(1024, 65533) // Yeah, sometimes this will fail, sorry :P
	httpPort = strconv.Itoa(p)
	wsPort = strconv.Itoa(p + 1)
	aegon := runMinimalGeth(t, "--miner.etherbase", "0x8605cdbbdb6d264aa742e77020dcbc58fcdce182",
		"--ipcpath", ipc,
		"--http", "--http.port", httpPort,
		"--ws", "--ws.port", wsPort)
	t.Run("ipc", func(t *testing.T) {
		waitForEndpoint(t, ipc, 3*time.Second)
		testAttachWelcome(t, aegon, "ipc:"+ipc, ipcAPIs)
	})
	t.Run("http", func(t *testing.T) {
		endpoint := "http://127.0.0.1:" + httpPort
		waitForEndpoint(t, endpoint, 3*time.Second)
		testAttachWelcome(t, aegon, endpoint, httpAPIs)
	})
...
func testAttachWelcome(t *testing.T, aegon *testaegon, endpoint, apis string) {
	...
	attach.SetTemplateFunc("datadir", func() string { return aegon.Datadir })
