...
func loadConfig(file string, cfg *aegonConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func makeConfigNode(ctx *cli.Context) (*node.Node, aegonConfig) {
	// Load defaults.
	cfg := aegonConfig{
...
func applyMetricConfig(ctx *cli.Context, cfg *aegonConfig) {
